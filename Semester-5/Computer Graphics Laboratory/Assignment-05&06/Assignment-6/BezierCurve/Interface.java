...
public class Interface extends Applet implements ActionListener, MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    Label L2 = new Label("Precision :");
    Label L3 = new Label("Give the control-points like 'x1,y1,x2,y2,...,xn,yn' :");
    Label L4 = new Label("Change size of Point :");
    ...
    public void init() {
        ...
        add(L3);
        T3.setText("-9,0,-5,6,-1,8,2,4,4,-3,9,-6,13,-2");
        add(T3);
    ...
    public void paint(Graphics G) {
        ...
        for(int i=0;i<BezierCurve.n;i++) {
            a.plot(G,BezierCurve.cX[i],BezierCurve.cY[i],Color.YELLOW);
        }
	...
	public void actionPerformed(ActionEvent e) {
        BezierCurve.n = Integer.parseInt(T1.getText());
        BezierCurve.Precision = Double.parseDouble(T2.getText());

        String str = T3.getText();
        Coord = str.split(",");
        for(int i=0;i<BezierCurve.n;i++) {
            BezierCurve.cX[i] = Integer.valueOf(Coord[2*i]);
            BezierCurve.cY[i] = Integer.valueOf(Coord[(2*i)+1]);
        }

		if(e.getSource() == B1) {
            if(Coord.length != 2*BezierCurve.n) {
    ...
    public void mouseEntered(MouseEvent m) {
        showStatus("This is an Intellectual Property of Aratrika Pal. Mouse is in Applet window. Scroll up to Zoom-in and scroll down to Zoom-out.");
    }
    
    public void mouseExited(MouseEvent m) {
        showStatus("This is an Intellectual Property of Aratrika Pal. Mouse exited Applet window.");
    }
