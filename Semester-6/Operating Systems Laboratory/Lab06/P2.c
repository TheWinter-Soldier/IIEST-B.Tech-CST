...
#include <sys/types.h>  //for kill(2), fork(), ftok(), semget(), semop() and semctl()
...
int main() {
    ...
    if(pid == -1) {
	...
	else if(pid==0) {   //child process task
        if(!isSetInitialized()) {
        ...
        while(1) {
            ...
            P(semid,1);
	        fprintf(stdout,"I am P2 section 1!\n");
            sleep(1);
            V(semid,2);
        }
	} 
	else {              //parent process task
		if(!isSetInitialized()) {
            printf("Semaphore set is not initialized. Initializing it:\n");
            for(int i=0;i < SET_CARDINALITY-1;i++) {
                if(i != 3) {
        ...
        while(1) {
            checkSem(SET_CARDINALITY-1);
            P(semid,3);
	        fprintf(stdout,"I am P2 section 2!\n");
            sleep(1);
            V(semid,0);
        }
	}
