...
struct student {
    ...
    void *data_p;       //The pointer to a memory location
...
int main(int argc,char *argv[]) {
    ...
    if(cpid == -1) {
        ...
        for(int i=0;i<n;i++) {
            printf("Give Roll-number, Name and CGPA of %d",i+1);
            if((i%10) == 0) {
                printf("st student ---\n");
            }
            else if((i%10) == 1) {
            ...
            printf("Give the CGPA: ");
            scanf("%lf",(double *)ss[i].data_p);
        }
        ...
        for(int j=100-n;j<100;j++) {
            printf("%d\t\t%s\t\t%.2lf\n",ss[j].roll,ss[j].name,*(double *)(ss[j].data_p));
        }
